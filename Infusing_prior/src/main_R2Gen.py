...
from src.models.r2Gen import  R2Gen
...
def parse_agrs():
    ...
    #Extra argument for applying custum labeler
    parser.add_argument('--infuse_prior', type=bool, default=False, help='whether include comparison prior in the model or not')
    parser.add_argument('--labeled_report_path', type=str, default='data/labeled_iu_reports.csv', help='the path to the directory containing the data.')
    parser.add_argument('--inc_classifier', type=bool, default=False, help='whether include classifier instead of rule-based labeler(ground truth) or not')
    parser.add_argument('--classifier_model_path', type=str, default='data/model_best.ckpt', help='the path to saved classifier model.')    
...
def main():
    ...
    model = R2Gen(args, tokenizer, criterion)
    ...
    trainer.evaluate(model=model, datamodule=pl_data_module, ckpt_path=None)
    #trainer.evaluate(model=model, datamodule=pl_data_module,\
    #            ckpt_path="/cluster/work/medinfmk/ARGON2/ARGON-main/results/mimic_cxr/r2gen/model_best.ckpt")
