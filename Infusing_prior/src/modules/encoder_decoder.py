...
from src.modules.att_model import pack_wrapper, AttModel
...
class DecoderLayer(nn.Module):
    ...
    def tmp_fun(self,x, tgt_mask):
        return self.self_attn(x, x, x, tgt_mask)

    def tmp_fun_src(self,x,m,src_mask):
        return self.src_attn(x, m, m, src_mask)

    def forward(self, x, hidden_states, src_mask, tgt_mask, memory):
        m = hidden_states
        def temp_self_attn(x):
            return self.self_attn(x, x, x, tgt_mask)

        def temp_src_attn(x):
            return self.src_attn(x, m, m, src_mask)

        x = self.sublayer[0](x, temp_self_attn, memory)
        x = self.sublayer[1](x, temp_src_attn , memory)
...
class EncoderDecoder(AttModel):
    ...
    def make_model(self, word_embeddings):
        c = copy.deepcopy
        ...
        model = Transformer(
            Encoder(EncoderLayer(self.d_model, c(attn), c(ff), self.dropout), self.encoder_num_layers),
            Decoder(
                DecoderLayer(self.d_model, c(attn), c(attn), c(ff), self.dropout, self.rm_num_slots, self.rm_d_model),
                self.decoder_num_layers),
            self.identity,
            nn.Sequential(word_embeddings, c(position)),
            rm)
    ...
    def __init__(self, args, tokenizer):
        ...
        self.word_embeddings = Embeddings(self.d_model, tgt_vocab)

        self.model = self.make_model(self.word_embeddings)
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None, comparison =None):
        ...
        tgt_emb = self.word_embeddings(seq)
        p = torch.softmax(out,dim=-1)
        weighted_tgt = torch.mul(p,tgt_emb)
        return outputs, weighted_tgt
