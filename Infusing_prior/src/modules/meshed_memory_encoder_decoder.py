...
class WordEmbeddings(nn.Module):
    def __init__(self, d_model, vocab):
        ...
        model_name_or_path = os.path.join(f"/cluster/work/medinfmk/ARGON/containers/models/w2v/glove")
...
class Transformer(nn.Module):
    ...
    def forward(self, src, tgt, src_mask, tgt_mask, comparison):
        # 128*16*512, 11*16, None, 11*11
        temp = self.encode(src, src_mask)
        if comparison != None:
            pad = torch.zeros(temp.shape, device = temp.get_device())
            for i, com in enumerate(comparison):
                if com != 0:
                    pad[i,:,:] = com    
            temp = temp + pad 
            
        return self.decode(temp, src_mask, tgt, tgt_mask)
    ...
    def decode(self, hidden_states, src_mask, tgt, tgt_mask):
        #print("#"*108)
        #print("In meshed_memory_encoder_decoder.py Transformer()- decode()")
        #print("hidden_states, src_mask, tgt, tgt_mask", hidden_states.shape, src_mask, tgt.shape, tgt_mask.shape)
        #print("#"*108)            
...
class DecoderLayer(TransformerDecoderLayer):
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None, tgt_key_padding_mask=None,
        ...
        #######
        if memory.shape[2]!=tgt.shape[1]: 
            memory=memory.reshape(memory.shape[0],98,-1,memory.shape[3])
        ...
        for i in range(memory.shape[0]):
            if len(memory.shape) == 5:
                ...
                #print("#"*108)
                #print("In meshed_memory_encoder_decoder.py DecoderLayer()- forward()")
                #print("self.multihead_attn", tgt.shape, memory[i].shape, memory[i].shape, memory_mask, memory_key_padding_mask)
                #print("#"*108)  
...
class EncoderDecoder(AttModel):
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None, comparison =None):
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, seq)
        out = self.model(att_feats, seq, att_masks, seq_mask, comparison)
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        ...
        #print("#"*108)
        #print("In meshed_memory_encoder_decoder.py EncoderDecoder()- core()")
        #print("memory, mask", memory.shape, mask, ys.permute(1,0).shape, self.generate_square_subsequent_mask(ys.size(1)).to(memory.device).shape)
        #print("#"*108)               
