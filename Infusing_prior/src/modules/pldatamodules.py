...
class plDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if self.args.dataset_name =='iu_xray':
            ...
            self.train= MimiccxrSingleImageDataset(self.args, self.tokenizer, 'train', transform=transform_image_ifcc('train', self.args),
                                                       limit_length=self.N)
            self.validation = MimiccxrSingleImageDataset(self.args, self.tokenizer, 'val', transform=transform_image_ifcc('val', self.args),
                                                 limit_length=self.N)
            self.test = MimiccxrSingleImageDataset(self.args, self.tokenizer, 'test', transform=transform_image_ifcc('test', self.args),
                                                 limit_length=self.N)
        else:
            raise ValueError
    ...
    def collate_fn(self, data):
        if self.args.infuse_prior:
            comparison, images_id, images, reports_ids, reports_masks, seq_lengths = zip(*data)
        else:
        ...
        if self.args.dataset_name =='iu_xray':
            images_id = torch.LongTensor(images_id)

        if self.args.infuse_prior:    
            return torch.LongTensor(comparison), images_id, images, torch.LongTensor(targets), torch.FloatTensor(targets_masks)
        else:
