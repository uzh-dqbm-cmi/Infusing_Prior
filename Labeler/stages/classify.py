...
class ModifiedDetector(neg_detector.Detector):
    ...
    """
    def __init__(self, key_phrase_path):
        self.key_patterns = ngrex.load(key_phrase_path)
        ...
        try:
            ...
            for loc in locs:
                for node in neg_detector.find_nodes(g, loc[0], loc[1]):
                    # Match keyphrase rules.
                    key_m = self.match_key_phrase(g, node)
                    if key_m:
                        yield POSITIVE, key_m, loc

    def match_key_phrase(self, graph, node):
...
class Classifier(object):
    ...
    def __init__(self, key_phrase_path, verbose=False):
        ...
        self.detector = ModifiedDetector(key_phrase_path)
